...
OPEN_TYPE_oer_get(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    int is_choice = (elm->type->elements_count > 0);
    ...
    if(is_choice || is_ioc_inline) {
        ...
        if(*memb_ptr2 == NULL) {
            ...
            if(is_choice && CHOICE_variant_set_presence(elm->type, *memb_ptr2, 0) != 0) {
                ASN__DECODE_FAILED;
            }
        }
    } else {
    ...
    if(elm->type->elements_count > 0) {
        ...
        if(is_choice && elm->type->elements && selected.presence_index > 0 
